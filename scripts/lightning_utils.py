...
def get_data_name(config: dict) -> str:
    """Return dataset name from either data.name or legacy data_name keys."""
    data_section = config.get('data', {})
    data_name = data_section.get('name') if isinstance(data_section, dict) else None
    if data_name is None:
        data_name = config.get('data_name')
    if data_name is None:
        raise ValueError("Expected 'data.name' or 'data_name' in the config.")
    return data_name


def get_datamodule_class(config_path: str):
    """Resolve the DataModule class registered under the dataset name."""
    config = parse_config(config_path)
    data_name = get_data_name(config)
    try:
        return DATASET_REGISTRY.get(data_name)
    except KeyError:
...
def prepare_config_for_lightning(config_path: str) -> str:
    ...
    config = parse_config(config_path)
    data_section = config.get('data')
    if isinstance(data_section, dict) and 'name' in data_section:
        del data_section['name']
...
def process_resume_arg():
    """Convert --resume to --trainer.ckpt_path format."""
    ...
    sys.argv.pop(idx)
    sys.argv.pop(idx)
    sys.argv.extend(['--trainer.ckpt_path', ckpt_path])
