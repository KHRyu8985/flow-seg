...
class SegDiffUNet(nn.Module):
    ...
    def __init__(self, dim, image_size, mask_channels=1, input_img_channels=1, init_dim=None,
                 ...
                 attn_heads=4, resnet_block_groups=8, rrdb_blocks=3, residual=False,
        ...
        self.output_activation = nn.Tanh() if apply_tanh else nn.Identity()

    def forward(self, x, time, cond):
        # Store initial input for residual connection
        x_init = x if self.residual else None
...
class MedSegDiffUNet(nn.Module):
    ...
    def __init__(self, dim, image_size, mask_channels=1, input_img_channels=1, init_dim=None,
                 ...
                 skip_connect_condition_fmap=False, residual=False,
                 apply_tanh: bool = False):
        super().__init__()
